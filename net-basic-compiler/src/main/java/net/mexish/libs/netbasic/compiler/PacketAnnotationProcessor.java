...
public final class PacketAnnotationProcessor extends AbstractProcessor {
    ...
    private void generateDispatcher(final @NonNull String targetPackage,
                                    final @NonNull TypeElement stateClass,
                                    final @NonNull List<HandlerInfo> handlers) {
        ...
        val packetToHandlers = new LinkedHashMap<TypeElement, List<HandlerInfo>>();
        for (val handler : handlers) {
            packetToHandlers.computeIfAbsent(handler.packetClass, v -> new ArrayList<>()).add(handler);
        }

        for (val entry : packetToHandlers.entrySet()) {
            val packetClass = entry.getKey();
            val packetHandlers = entry.getValue();

            dispatchMethod.addCode("case $T p -> ", ClassName.get(packetClass));
            dispatchMethod.beginControlFlow("");
            dispatchMethod.beginControlFlow("for (Object module : logicModules)");

            for (val handler : packetHandlers) {
            ...
            dispatchMethod.endControlFlow(); // closes case
        }
